  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        {loading ? (
          <svg className="w-3 h-3 animate-spin text-emerald-400" viewBox="0 0 24 24" fill="none">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" />
          </svg>
        ) : (
          <svg viewBox="0 0 20 20" fill="none" className={`w-3 h-3 text-gray-400 transition-transform ${open ? "rotate-180" : ""}`}>
        ...
        <div className="absolute right-0 top-full mt-2 w-44 bg-[#0d2318] border border-emerald-900 rounded-xl shadow-2xl shadow-black/50 overflow-hidden z-50">
          {LANGUAGES.map((lang) => (
            ...
            >
              ...
              <span>{lang.name}</span>
              {language === lang.code && (
                <svg viewBox="0 0 20 20" fill="none" className="w-4 h-4 ml-auto">
