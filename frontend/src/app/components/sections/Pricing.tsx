        ...
        {submitted ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-400">
              ...
              <span className="text-[#2d9e6b] font-semibold">{plan.name}</span>{" "}
              {t("quote.successPlan")}
            </p>
            <button
              onClick={onClose}
              className="mt-6 bg-[#2d9e6b] text-white px-6 py-3 rounded-xl font-semibold hover:bg-[#1a7a50] transition-all"
            >
              {t("quote.close")}
            </button>
          </div>
          ...
          <>
            <div className="mb-6">
              <span className="text-xs font-semibold text-[#2d9e6b] uppercase tracking-wider">
                {t("pricing.getQuote")}
              </span>
              <h3 className="text-2xl font-bold text-white mt-1">
                {plan.name} {t("quote.plan")}
              </h3>
              <p className="text-gray-400 text-sm mt-1">
                {plan.setup} {t("pricing.setup")} · {plan.monthly}
                {t("pricing.perMonth")}
              </p>
            </div>
              ...
              <div key={plan.name} className="relative pt-5">
                ...
                >
                  ...
                  <div className="text-center mb-8">
                    ...
                    <p
                      className={`text-sm mb-4 ${isActive ? "text-blue-100" : "text-gray-400"}`}
                    >
                      {plan.tagline}
                    </p>
                    <div className="space-y-1">
                      <p className="text-3xl font-bold">
                        {plan.setup}{" "}
                        <span
                          className={`text-lg font-normal ${isActive ? "text-blue-200" : "text-gray-400"}`}
                        >
                          {t("pricing.setup")}
                        </span>
                      </p>
                      <p className="text-2xl font-semibold">
                        {plan.monthly}
                        <span
                          className={`text-base font-normal ${isActive ? "text-blue-200" : "text-gray-400"}`}
                        >
                          {t("pricing.perMonth")}
                        </span>
                      </p>
                  ...
                  <div className="space-y-3 mb-8">
                    {plan.features.map((feature, i) => (
                      <div key={i} className="flex gap-3 items-start">
                        ...
                        <p
                          className={`text-sm leading-relaxed ${feature.included
                            ? "text-white"
                            : isActive
                              ? "text-emerald-300 opacity-50"
                              : "text-gray-500"
                            }`}
                        >
                          {feature.text}
                  ...
                  <button
                    ...
                    className={`w-full py-4 rounded-xl font-semibold transition-all ${isActive
